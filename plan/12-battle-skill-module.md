...
> **⚠️ 已部分被 M19 取代（2026-06-24）**：使用者拍板**放寬單招硬約束 → 寶可夢式多招式制**。本檔「技能＝非傷害輔助層、單招不放寬」
> 的核心前提**已過時**——怪物招式 loadout/訓練/學習/繼承（原 M8.0/M8.a/M8.b/M8.c）改由 **M19（`plan/17-mobie-multimove-skills.md`）**承載，
> 變化招進招式槽、攻擊招可多個。**M12 仍有效的剩餘部分**：合體技（§4，擴 M9 連鎖）、對手 Encounter Skill Profile（§5）、`fieldState` 統一（§6）。
> 讀本檔時以 `plan/17` 與 CLAUDE.md「Move system is now MULTI-MOVE」為現行真相，下列「守單招」字眼視為歷史脈絡。
>
...
> ~~**硬約束守住**：CLAUDE.md「每隻寶可夢單一專屬招式（Mezastar 風）」**不放寬**~~（**已於 2026-06-24 放寬，見上方 banner 與 M19**）——
> 專屬 QTE 大招的身分定位由**星擊 finisher** 承載；合體技/對手 profile/fieldState 仍沿用 M6 §0 的 S1–S8 縫 + `ext` 注入 + §0.4 相位契約。
