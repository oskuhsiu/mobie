...
| `EXT.0-enhancement-report.md` | **強化報告（遊戲性/視覺/操作性）總覽**——M0–M22 後下一輪 13 項三維評估＋優先級分層＋紅線＋EXT 系列地圖。**EXT 系列真相來源**（四方圓桌 2 輪全 agree，結論 `.claude/agent-chat/session-20260625-092816/`）|
| `EXT.1-in-battle-juice.md` | **局內爽感（B1 打擊感＋C1 Haptics＋C3 圖示音效＋B4 Sprite動態/轉場/修4置中overlay＋C2 脈衝引導）**——純 display 零碰 reducer；`damageApplied` 事件已備數字/會心/效果絕佳；含 `cinematicCoordinator` 演出協調 seam 地基 |
| `EXT.2-star-strike-cinematic.md` | **星擊電影化（B2）**——letterbox/慢動作/運鏡/全螢幕 cut-in；消費 EXT.1 的 coordinator；`runStarStrike` 簽名不動，純演出 |
| `EXT.3-terrain-weather-visuals.md` | **地形/天氣視覺化（B3）**——M13 地形資料映射到 R3F 場景（雨/晴/沙/電場程序粒子）；讀 `field.terrainEffects`、reducer 不動 |
| `EXT.4-status-ailments-module.md` | **狀態異常 module（A1，唯一動 reducer）＋頭目階段 rider（A4）**——`modules.statusAilments` 預設 off 零殘留；持續型走 S4、行動阻斷型加 `actionGate`/`postAttack` 新 seam；ailment 命名避開既有 StatusEffect；回放耦合治理 |
| `EXT.5-meta-loop.md` | **養成 meta loop（A2 貨幣商店＋A3 任務每日）**——後段；新 store slice（仿 bagStore）＋商店/任務 UI；不碰 reducer、經濟不偷改捕獲難度；順手把 wallet/quests 納入 `.save` |
