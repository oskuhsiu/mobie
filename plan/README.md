...
| `14-roadmap-m6-m8.md` | **M6~M8 整合路線圖**（22 項依類型歸 7 家族 + 依賴排序 P0–P6 實作順序 + 完整性檢視；找出道具/特性/技能等跨里程碑合併點） |
...
| **M6** | 延伸系統群（可選掛載）：M6.0 掛載地基+相位契約 → wave-1 羈絆/道具/進化/連鎖/連勝塔 `09` → wave-2 Grade/特性/圖鑑成就/孵化/Ascension `10` | 🧩 可選式深度 |
| **M7** | 核心玩法擴充：地形系統（影響戰力/混合/隨機）+ 競技場/野外模式分流 + 野外意外×5 → `11-terrain-modes-accidents.md` | 🗺️ 地形與野外 |
| **M8** | 戰鬥技能大模組：技能 loadout（訓練/學習/繼承）+ 合體技（chain variant＋施放效果）+ 對手 profile → `12-battle-skill-module.md`；內容補完 roadmap → `13-content-roadmap.md` | ⚔️ 技能與合體技 |
