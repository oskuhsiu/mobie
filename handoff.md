...
## 5. 下一步（✅M18+M19+M16+M11野外意外；▶ 續 M11 連勝塔/Ascension → 稀疏初始配招）
M1–M10 完成並 CDP 驗證；**M18 改名 + M19 多招式制（a–f）+ M16 資訊卡（a–c）+ M11 野外意外×5 完成，321 測試全綠**。**執行序：M18 → M19 → M16 → M11**（M21 戰鬥特效並行新設計，不在本輪目標內）。
- **▶ 目標進行中（/goal 完成 m19/m16/m11）**：M19✅ M16✅ M11野外意外✅；**續 M11 連勝塔/Ascension（最後一塊）**。
...
- **✅ M19.e 招式訓練所 + SP（2026-06-24，CDP 驗證）**：`skillPointsStore`（`mobie.skillpoints.v1`，帳號級，M17/M19 共用、UI 分池）；ResultScreen 勝利給 SP（野外 boss 2+lv/10、競技場 1，塔 SP 預留 M11）；`MoveTrainerModal`（Title「📖 招式」，圖鑑改 📚）學新招（花 SP、teachable 未學、依威力 tier 計價）/ 調 loadout（點切換、上限 4 至少留 1）；rosterStore `learnMove`（物化完整已學集 union）/`setEquippedMoves`（⊆已學∪slot0、截≤4、保證非空）；`grantBattleExp` 升級領悟（同種族 union learnedMoveIds、`lastMoveLearns` 追蹤）；learnset `effectiveLearnedMoves`/`newlyLearned`/`teachableNotLearned`。`learnedMoveIds` 語意＝物化後存完整已學集（含等級領悟+訓練）。+7 vitest。**CDP**：Lv.20 妙蛙種子→招式所→學瞑想 SP10→8、loadout 上限 4 強制、零 error。**follow-up**：moveLearned 結算提示 UI、招式回憶顯式分頁（目前憶/忘併入 loadout 點切換）。
