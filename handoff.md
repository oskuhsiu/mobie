...
## 1. 現況一句話
**M1.x + M2/M3/M5–M11 + M18/M19/M16/M17/M21/M22 全部完成並 Chrome CDP 驗證**（367 測試 / typecheck / build 全綠）。
...
M1–M11 完成並 CDP 驗證；**M18 改名 + M19 多招式制（a–f）+ M16 資訊卡（a–c）+ M17 Partner 玩家技能（a–d）+ M11 野外意外×5/連勝塔/Ascension + M21 戰鬥技能特效 + M22 增強互動性（a–e MVP）完成，367 測試全綠**。**執行序 M18 → M19 → M16 → M11 → M21 → M22 → M17 已收束**（稀疏初始配招、M22.f–j 互動 backlog 是後續目標）。
