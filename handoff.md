...
## 1. 現況一句話
**M1.x + M2/M3/M5–M11 + M18/M19/M16/M17/M21/M22 全部完成並 Chrome CDP 驗證**（372 測試 / typecheck / build 全綠）。
> **最新（2026-06-24 晚，玩測回饋 5 修，已 push origin/main）**：見 §5 開頭「玩測回饋 UX 修正」。
...
- 多個里程碑畫面都經 **三/四方 agent-chat 設計審查**（P0/P1 已落地，conclusion 在各 session）。**下一步：稀疏初始配招，或 M22.f–j 互動 backlog；M4（MediaPipe 體感）使用者目前略過（M22 手勢層已是其觸控前身、同源）。M17 Partner 玩家技能已完成（見 §5）。M20 DQ 來源已棄置（無官方 API）。**
...
> **已知 follow-up（不阻塞）**：M11 地形突變已落地（terrainShift 改 field.current，野外意外）；M12 fieldState 再補三子欄。
>
...
> **已知 follow-up（不阻塞）**：完整 `pendingCaptures` 斷線復原 transaction（自用單機刻意簡化為同步轉蛋）；`mz.meta.v1`/`mz.incubator.v1`/`mz.itembag.v1` 尚未進 `.save` 匯出（roster 內 heldItemId 已含）；進化解鎖技能槽 / 蛋帶技能 接點待 M12；塔來源 egg 待日後擴充，塔 SP 已由 M11 `floorReward` 接上。
>
...
> **Mobie 資訊卡 + Partner 技能系 + 全面改名 = M16–M18（2026-06-24 設計脈絡；M16/M18 已落地，M17 待後續）**：使用者實玩回饋（戰鬥中看不到自己夥伴的型別/技能/數值）+「玩家與 mob 是**夥伴而非從屬**」理念 + 把「pokemon」字眼全改 **mobie**。設計+決策全文**已拆三檔**（2026-06-24）：`plan/16-mobie-info-card.md`（M16）/`plan/19-partner-player-skills.md`（M17）/`plan/20-rename-to-mobie.md`（M18）。共同身分軸線：「你的 mob 是夥伴，這個遊戲叫 Mobie」。
...
> **M17 Partner 技能系（提前並重定位 M12 核心，複用 M7 S1–S8 引擎 + M8 fieldState，戰鬥機制零 reducer/engine 改動）**：①**自動技能**＝既有 hook 模組（鼓舞=S3 pinch / 守護=S3 guard / 疾風=S1 statMod，`partnerSkills.ts` push `MODULE_REGISTRY`、讀暫態 `equippedSkillIds` 自行分流，同道具/特性路）。②**主動槽**（1 個、每場一次、手動鈕）＝**純顯示層**的看穿（設 `revealedFoes` + 揭露演出，不進 reducer/不耗回合/對手不回擊），接 M16 的卡。③**完整 SP 訓練經濟**（`mobie.skillpoints.v1` 錢包、boss 勝利給 SP、`PartnerSkillModal` 訓練所學/裝、第 2 槽 SP/等級解鎖；塔 SP 已由 M11 `floorReward` 提供、進化解槽預留 M10）。`OwnedUnit` 只加 canonical `learnedSkillIds/equippedSkillIds`。**M12 剩餘子項（合體技/對手 profile/孵化繼承）續留原里程碑。**
> **M18 全面改名 → Mobie（取代並擴大 M15；a–d 已落地，e 待使用者改 repo 目錄/remote）**：**分類精準改名非一鍵替換**。詞彙全用 `mobie`（`BattlePokemon`→`BattleMobie`、`PokemonSprite`→`MobieSprite`、UI「寶可夢」→「Mobie」）。範圍 src 約 137 處/32 檔 + 「寶可夢」32 處/24 檔 + 2 檔名 + 品牌字串 + docs + repo 目錄。**⚠️ 絕不可改**：`artwork()` helper / gen_dex 的 `raw.githubusercontent.com/PokeAPI/.../pokemon/...` URL、外部服務名 `PokéAPI`、物種 zh-Hant 正典名。存檔 key `mz.*`/`mz-*` → `mobie.*`/`mobie-*` + 一次性 `migrateKeys()`，`.save` 舊欄位向後相容匯入。實際執行已改為 M18 先做（見下）。
...
- `input/qte.ts`（`qualityFromPointer` seam）；**`input/gestures/`（M22 純手勢層：swipeFromPointer/circleProgress/holdCharge/rhythmTaps + angleTo/wrapAngle/sweptAngle/swipeThrowValid/beatSchedule/matchTapsToBeats/rhythmAccuracy，輸出單一純量、與 M4 體感同源）**。
- `ui/`：screens（Title[**含 ⚙️設定/🎒隊伍 入口**]/RegionSelect[**含競技場入口 + M8 地形提示膠囊 + M11 🗼連勝塔入口**]/Encounter[**M11 rareBoss/lucky/supply**]/CardSelect[對手條+剋弱徽章+推薦+**羈絆 tag**]/Battle[**resolveTurn 傳 ext+terrainMultiplier+wildEvents、applyBattlePrep 套 prep、特性/道具徽章、heal 演出、M8 開場地形揭示 + 常駐 TerrainChip；塔戰中性場；M22 star-orb 點擊依 starMode 開蓄力/節奏手勢**]/Result[**WinView 捕獲 vs ArenaWinView 純經驗 vs TowerView 無捕獲，依 `canCaptureIn`/tower 分流；M22 WinView 依 capMode 接 SwipeThrow/CircleSeal**]/TowerSetup[選 3 隻 + Ascension]）、components（HpBar/TypeBadge/MobieSprite/TimingBar/IndividualInfo/MobCard/**SettingsModal[含 M22 🕹互動偏好三態]/TeamModal/CaptureGestures〔SwipeThrow·CircleSeal〕/StarStrikeGestures〔HoldChargeRing·RhythmTap〕**）、`styles/global.css`。

## 5. 下一步（✅M18+M19+M16+M17+M11+M21+M22 全完成；▶ 稀疏初始配招 / M22.f–j backlog）
M1–M11 完成並 CDP 驗證；**M18 改名 + M19 多招式制（a–f）+ M16 資訊卡（a–c）+ M17 Partner 玩家技能（a–d）+ M11 野外意外×5/連勝塔/Ascension + M21 戰鬥技能特效 + M22 增強互動性（a–e MVP）完成，372 測試全綠**。**執行序 M18 → M19 → M16 → M11 → M21 → M22 → M17 已收束**（稀疏初始配招、M22.f–j 互動 backlog 是後續目標）。

> **✅ 玩測回饋 UX 修正（2026-06-24 晚，CDP 真機驗證、已 push；commit 4e85731…9ef72ac）**：使用者實玩回報 5 點全修：
> ① **隊伍頁看不到自己 Mobie 詳情** → `TeamModal` 每隻 Mobie 加可點區（「ℹ️ 詳情」），由 `buildBattleMobie(ownedToCard(u))` 建檢視用實例（補 `abilityForType` 特性 id）以 owner 全顯開 `MobCard`；MobCard 置 backdrop **sibling**（fragment）避免冒泡關掉隊伍頁。
> ② **首頁工具列開始遊戲後消失** → 抽出共用 `src/ui/components/ToolsMenu.tsx`（工具鈕 + 11 個 lazy overlay + meta/incubator 紅點），`TitleScreen` 與 `RegionSelectScreen`（遊戲中樞）共用；RegionSelect 底部 `.region-tools` + 加「🏠 首頁」鈕；`gameMachine` regionSelect 加 `BACK: 'title'`。
> ③ **設定『連勝塔』標敬請期待誤導** → 連勝塔是 RegionSelect 進入的**遊戲模式**、非可選掛載模組（不在 `MODULE_REGISTRY`、toggle 無作用），從 `ModuleId`/`MODULE_IDS`/`MODULE_META` 移除（測試改用 `evolution` 當範例 id）。`EggSource`/`InteractSurface` 的 `'tower'` 是不同概念、刻意保留。
> ④ **QTE bar 在最底看不到不好點** + ⑤ **連打時間太短（950ms）** → 計時類輸入（qte/mash/statusQte/defenseQte/chainQte）抬到戰鬥區中央的 `.battle-action` 放大面板；QTE bar 加高 46→58px；`MASH_DURATION_MS` 950→**2800** 並加 CSS 倒數條。
> **過程抓到真 bug**：framer-motion 動 `scale/y` 會蓋掉 CSS `translate(-50%,-50%)`（CDP 量到面板 center X 跑到 86% 右溢）→ 改用「撐滿 + flex 置中的 `.battle-action-layer`」（不靠 transform 置中），量回 X50%/Y51%。
> **follow-up（已記 memory `framer-centering-overlays-followup`，未做）**：同一陷阱潛伏在另 4 個戰鬥 overlay（`star-orb`/`battle-banner`/`support-overlay`/`combo-overlay`，皆動 scale/y 卻靠 `translate(-50%)` 置中），可能輕微偏移，待比照 `.battle-action-layer`（或抽共用 `<CenteredOverlay>`）一次處理；修時用 CDP 量 getBoundingClientRect 驗證。

> **✅ 版號顯示 + commit 自動升版（2026-06-24 晚）**：版號真相＝`package.json`。`vite.config.ts` 把版號注入編譯期常數 `__APP_VERSION__`（型別宣告在 `src/vite-env.d.ts`），`TitleScreen` eyebrow 顯示「Personal Arcade · {裝置} · v{版號}」。**每次 commit 自動升 patch**：`.githooks/pre-commit` → `scripts/bump-version.mjs`（升 patch、同步 package.json 與 package-lock 自身版號、不動相依）→ `git add` 納入本次 commit；hook 路徑由 `package.json` 的 `prepare` script（`git config core.hooksPath .githooks`）於 `npm install` 自動設定（**新 clone 須跑一次 `npm install` 或手動 `git config`**）。已驗：commit 後 0.1.0→0.1.1，CDP 首頁顯示 v0.1.1。**注意：amend/rebase 會再升一次 patch**（自用可接受）。
- **✅ M17 Partner 玩家（訓練師）技能（2026-06-24，CDP 驗證；plan/19）**：純玩家帳號級工具、**不掛 OwnedUnit、戰鬥機制零 reducer/engine 改動**。`game/ext/partnerSkills.ts`（catalog：🔍 看穿 active/reveal cost0 起始、📣 訓練師加油 support/teamBuff SP6；純函式查表用共用 createLookup）；`store/playerSkillsStore.ts`（`mobie.playerskills.v1`，只存已花 SP 解鎖的 id，起始技能由 catalog `isPartnerSkillLearned`〔cost0〕視為恆習得）。**看穿**＝戰中主動鈕（`battleStore.revealedFoes`/`revealFoe` 沿用 M16、揭露對手 MobCard 深度）**不進 reducer/不耗回合/對手不回擊**；**訓練師加油**＝`battleStore.applyTeamStatuses` 灌注全隊增益到 `field.teamStatuses`（複用 M19.d StatusEffect，reducer 既有消費＝零改動）。**每場一次預算**＝`partnerUsed` component state（init 重置、不持久化）。`PartnerSkillModal`（Title「✨ 夥伴技能」）花 SP 解鎖、**與 M19 招式分頁分池顯示**（兩 modal/兩成本表）；`modules.partnerSkills` 設定 toggle（預設關、**非 seam 模組不入 MODULE_REGISTRY，同 tower 慣例**、UI gating only）。+7 vitest（護欄：無 damage/不寫 OwnedUnit、teamBuff 皆增益、起始恆習得、store 冪等）= 367 全綠。**CDP（SwiftShader）**：ON＝SP20→14 解鎖加油、戰中看穿揭露對手 3 招、加油全隊提升+用後 disabled、零 error；OFF＝行動列無夥伴鈕（零殘留）、零 error。**simplify/code-review（xhigh workflow）已修**：①`battleStore.applyTeamStatuses` 改走 reducer 匯出的純 `upsertStatus`（同 stat 取 max、不與變化招連乘爆表，守 M19.d 硬上限；+5 vitest）②看穿/加油 banner 主動清除（不再殘留）③`PartnerSkillModal.unlock` 加已習得 guard（spend 非冪等、防快速重複點擊重扣 SP）④rally 文案改「戰鬥中使用一次」（去除「開戰時」與實作不符）⑤getPartnerSkill 用共用 createLookup、移除死碼 STARTER_PARTNER_SKILL_IDS/測試專用 has()。**follow-up（不阻塞）**：看穿目前「每場一次、揭露當下 active 對手」——對手陣亡換上後無法再看穿新威脅（plan/19 待玩測「每場一次 vs 每隻一次」）；丟道具技能、全隊支援「回合中有界 action」變體（本輪只做一次性）；`mobie.playerskills.v1`/`mobie.skillpoints.v1` 尚未進 .save 匯出。
...
- **✅ M16 Mobie 資訊卡（2026-06-24，CDP 驗證）**：`MobCard`（吃 BattleMobie+owner+revealed，純 UI 零 reducer/engine/持久化）——立繪/名/Lv/異色/型別 + 招式 loadout row（名/型別/物理特殊變化/威力命中）+ 六維 mini-bar + IndividualInfo + 特性/道具；**自己(owner)/已看穿全顯、對手未看穿基本面+深度遮罩「🔍 看穿後揭露」**。接線：BattleScreen HpPlate/TeamTray、CardSelect ⓘ角標/foe-strip、Encounter ⓘ/縮圖。`battleStore.revealedFoes`（M16 恆空、M17 看穿 add）+`revealFoe`。CDP 三開啟點全驗、零 error。
- **✅ M19.f gen_dex learnset（2026-06-24，CDP 全 loop）**：gen_dex `buildLearnset`（slot0@L1 + 各屬性 3 tier 攻擊招依 **PokéAPI 真實 level-up 級數分位** + 變化招）/`buildTeachable`（型別全 tier + 全變化招）→ emit `learnset`/`teachableMoveIds` 進 species.ts（**重產只動 species.ts**）；運行時 learnsetOf/teachableOf 走產生檔（deriveLearnset 留 fallback）；simulation 玩家帶隨機 slotIndex 壓測多招式/變化招。CDP 全 loop 預設 roster 打完整場零 error。
- **✅ M19.e 招式訓練所 + SP（2026-06-24，CDP 驗證）**：`skillPointsStore`（`mobie.skillpoints.v1`，帳號級，M17/M19 共用、UI 分池）；ResultScreen 勝利給 SP（野外 boss 2+lv/10、競技場 1、塔層走 M11 `floorReward`）；`MoveTrainerModal`（Title「📖 招式」，圖鑑改 📚）學新招（花 SP、teachable 未學、依威力 tier 計價）/ 調 loadout（點切換、上限 4 至少留 1）；rosterStore `learnMove`（物化完整已學集 union）/`setEquippedMoves`（⊆已學∪slot0、截≤4、保證非空）；`grantBattleExp` 升級領悟（同種族 union learnedMoveIds、`lastMoveLearns` 追蹤）；learnset `effectiveLearnedMoves`/`newlyLearned`/`teachableNotLearned`。`learnedMoveIds` 語意＝物化後存完整已學集（含等級領悟+訓練）。+7 vitest。**CDP**：Lv.20 妙蛙種子→招式所→學瞑想 SP10→8、loadout 上限 4 強制、零 error。**follow-up**：moveLearned 結算提示 UI、招式回憶顯式分頁（目前憶/忘併入 loadout 點切換）。
...
>
> **M22 增強互動性設定開關（2026-06-24，✅ MVP〔a–e〕完成並 CDP 驗證；plan/22）**：使用者要求「設定打開『增強互動性』後多出操作（如集氣連點）給兒童參與感，不要只有點一下又點一下」。四方 agent-chat（Claude/gemini/codex/mistral）3 輪全體 agree：`.claude/agent-chat/session-20260624-132835/conclusion.md`。
> **✅ 已落地（a–e，+30 vitest = 360 全綠）**：`game/settings.ts` 新 `prefs.enhancedInteractivity`（`InteractMode`/`InteractSurface` + `defaultPrefs`/`migratePrefs` 向後相容〔缺欄→off、surfaces 只認顯式 false〕 + `setInteractModeIn` + selector `isEnhancedSurfaceEnabled`/`interactModeOf` + `INTENSITY_BY_MODE`）；`settingsStore.setInteractMode`（prefs **不入** ext/prep/postGrowth）；純手勢層 `src/input/gestures/`（swipeFromPointer/circleProgress/holdCharge/rhythmTaps + angleTo/wrapAngle/sweptAngle/swipeThrowValid/beatSchedule/matchTapsToBeats/rhythmAccuracy，比照 qte.ts、與 M4 同源）；捕獲 `CaptureGestures.tsx`（`SwipeThrow`/`CircleSeal`，WinView stage machine 加 aim/seal、`caught` 預先決定不動、「再搏一下」純演出寬限）；星擊 `StarStrikeGestures.tsx`（`HoldChargeRing`/`RhythmTap`，star-orb 點擊改開手勢、`runStarStrike` 簽名不動、不沿用 MashMeter）；`SettingsModal` 加「🕹 互動偏好」三態 selector。**reducer/engine/BattleEvent 零變更**；高頻值全 ref/rAF/DOM（rect 快取）、單次 onDone。**CDP（SwiftShader）四驗全零 error**：①設定三態+持久化 ②arcade 捕獲 aim→swipe→seal「✦畫圈封印」→收服成功 ③星擊 orb→RhythmTap→放招 ④off golden path `.gesture-surface`=null（零回歸）。+3 回歸 vitest（捕獲率不受 mode 影響）。simplify 收窄 starMode 訂閱回純字串 + normPt 快取 rect。**Backlog（未做）：M22.f–j 防禦下滑/攻擊節奏 `attackInputVariant`/遭遇撥草/孵化摩擦/連勝塔選路。**
> ──（以下為原設計脈絡，保留）──
...
> **MVP 只做兩個情緒峰**：①**捕獲**（lite=swipe 丟球 / arcade=swipe+畫圈封印，**`caught` 的 Math.random 預先決定一字不動**，手勢只決定演出+「再搏一下」純演出寬限、絕不改機率）②**星擊**（lite=長按蓄力環 / arcade=節奏三連點、不沿用攻擊連打、中斷逾時安全退場）。**Q1 裁定（codex/mistral 勝 gemini）：既有攻擊 MashMeter 不動，守 off=現狀一字不差**；arcade 連打換節奏＝後續旗標 `attackInputVariant`。
> **落地**：新 `src/input/gestures/`（swipeFromPointer/circleProgress/holdCharge/rhythmTaps 純函式、比照 `input/qte.ts` 輸出單一純量、高頻值全 ref/rAF/DOM）**與 M4 體感同源**；reducer/engine/event **零變更**（捕獲屬 ResultScreen、星擊屬 BattleScreen 顯示層）。統一 selector `isEnhancedSurfaceEnabled`。**驗收**：off 無新增 wrapper（零回歸）、lite/arcade 逾時/取消/多指/低 FPS 只 dispatch 一次、捕獲率==off。Backlog：防禦下滑/攻擊節奏/遭遇撥草/孵化摩擦/連勝塔選路。**切分 M22.a–e（+f–j backlog）見 CHECKLIST M22。**
...
- **M7/M8 收尾 follow-up（不阻塞、可順手）**：氣勢披帶需 post-damage 縫（改 engine）、威嚇需 onSwitchIn 縫（改 reducer 換人段）；M11 地形突變已用 M8 `field.terrainEffects.current` 分流落地。
