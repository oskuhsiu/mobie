...
## 5. 下一步（✅M18 改名 + ✅M19 全（a–f）完成；▶ 續 M16 資訊卡）
M1–M10 完成並 CDP 驗證；**M18 全面改名 → Mobie（a–d）完成並 CDP 驗證**；**M19 多招式制完整（a–f：資料模型 + reducer/engine additive + 四槽選招 UI + 變化招 + 招式訓練所/SP + gen_dex learnset）完成，310 測試全綠**。**2026-06-24 使用者拍板執行序：先改名（M18）→ M19 → M16 → M11**（M21 戰鬥特效為並行新設計，見下，不在本輪 M19/M16/M11 目標內）。
- **▶ 目標進行中（/goal 完成 m19/m16/m11，每階段 simplify/fix/commit）**：M19 全 ✅；續 **M16.a–c（MobCard 資訊卡）→ M11（模式長線意外）**。
