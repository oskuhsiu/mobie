...
## 1. 現況一句話
**M1.x + M2/M3/M5–M11 + M18/M19/M16/M21/M22 全部完成並 Chrome CDP 驗證**（360 測試 / typecheck / build 全綠）。
...
- 多個里程碑畫面都經 **三/四方 agent-chat 設計審查**（P0/P1 已落地，conclusion 在各 session）。**下一步：M17 Partner 技能系，或稀疏初始配招，或 M22.f–j 互動 backlog；M4（MediaPipe 體感）使用者目前略過（M22 手勢層已是其觸控前身、同源）。M20 DQ 來源已棄置（無官方 API）。**
...
## 5. 下一步（✅M18+M19+M16+M11+M21+M22 全完成；▶ M17 / 稀疏初始配招 / M22.f–j backlog）
M1–M11 完成並 CDP 驗證；**M18 改名 + M19 多招式制（a–f）+ M16 資訊卡（a–c）+ M11 野外意外×5/連勝塔/Ascension + M21 戰鬥技能特效 + M22 增強互動性（a–e MVP）完成，360 測試全綠**。**執行序 M18 → M19 → M16 → M11 → M21 → M22 已收束**（M17 Partner 技能系、稀疏初始配招、M22.f–j 互動 backlog 是後續目標）。
